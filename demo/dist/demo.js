typeof navigator === "object" && (function (Sentry, Shr) {
  ...
  // Setup tab focus
  const container$1 = document.getElementById('container');
  const tabClassName = 'tab-focus'; // Remove class on blur

  document.addEventListener('focusout', event => {
    if (!event.target.classList || container$1.contains(event.target)) {
      return;
    }
  ...
  document.addEventListener('keydown', event => {
    ...
    setTimeout(() => {
      ...
      if (!focused || !focused.classList || container$1.contains(focused)) {
        return;
      }
  ...
  const controls = {
    ...
    createTime(type, attrs) {
      ...
      toggleClass$1(container, this.config.classNames.matchTime, this.config.matchTime); // Reference for updates
  ...
  const ui = {
    addStyleHook(container) {
      toggleClass$1(container, this.config.selectors.container.replace('.', ''), true);
      toggleClass$1(container, this.config.classNames.uiSupported, this.supported.ui);
    },
  ...
  class Trim {
    ...
    createThumbTime() {
      ...
      toggleClass$1(leftThumb.timeContainer, classNames.trim.alwaysShowTimestamps, alwaysShowTimestamps);
      toggleClass$1(rightThumb.timeContainer, classNames.trim.alwaysShowTimestamps, alwaysShowTimestamps);
    }
    ...
    update() {
      ...
      toggleClass$1(this.player.elements.container, this.player.config.classNames.trim.enabled, this.enabled);
    } // Load config after changing of source (only do this for sources which have sync points)
