...
| `trim`               | Object                     | `{ enabled: false, closeEditor: true, maxTrimLength: -1, offsetContainer: false }`                                             | `enabled`: Whether to enable the video trimming tool. `closeEditor`: On exit of the trimming tool should the editor be exited. `maxTrimLength`: Specify a maximum trimming length in seconds. `offsetContainer`: When inserting the trimming window, set the current time to be the mid point.                                                                                                          |
